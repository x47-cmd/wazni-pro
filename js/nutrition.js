/* Liyaqti Nutrition Intelligence Pro - Premium V30 Architecture */
console.log("Liyaqti Nutrition Intelligence Pro V30 loaded");
...
/* =========================
   ...
   ملاحظة: أرقام المطاعم تقديرية وقد تختلف حسب الحجم والتخصيص
========================= */
const seedFoods=[
...
function renderSmartFoodSearch(){
  ...
  <div class="niFoodResults">
    ...
    `).join("")}
  </div>`:`<div class="niEmpty small">ما حصلت أكلة. جرّب كلمة أبسط مثل: دجاج، رز، برجر، بيتزا، ماك.</div>`;
}
...
function filterFoodCat(cat){
  ...
  box.innerHTML=foodRows(list.slice(0,160));
}
...
function openFoodModal(i=null){
  ...
  <div class="niModalBg">
    <div class="niModal">
      <div class="niModalHead">
        <h3>${x?"تعديل طعام":"إضافة طعام"}</h3>
        <button onclick="closeFoodModal()">×</button>
      </div>
      ...
      <div class="niForm">
        ...
        <div>
          ...
          <select id="fQuality">
            ...
            <option value="high_sodium">صوديوم عالي</option>
            <option value="high_fat">دهون عالية</option>
            <option value="high_sugar">سكر عالي</option>
          </select>
        </div>
      </div>
...
function drawOverviewCharts(){
  ...
  if(s.cal&&document.getElementById("calChart")){
    nutritionCharts.cal=new Chart(calChart,{
      ...
      data:{
        labels:["مستهلك","متبقي"],
        datasets:[{data:[s.cal,Math.max(0,NS.calories-s.cal)],borderWidth:0}]
      },
      options:{
        cutout:"72%",
...
function destroyCharts(){
  Object.values(nutritionCharts).forEach(c=>{
...
function injectNutritionStyle(){
  ...
  .niHero h2{font-size:21px;margin:5px 0 4px;font-weight:950}
  ...
  .niExecDash{display:grid;grid-template-columns:repeat(4,1fr);gap:8px}
  .niExecDash div,.niSummary,.niSearch,.niCard,.niStrategic{background:var(--card);border:1px solid var(--line);border-radius:23px;padding:14px;box-shadow:0 9px 22px #0000000d;overflow:hidden}
  .niExecDash small{display:block;color:var(--muted);font-size:10.5px;font-weight:900}
  .niExecDash b{display:block;color:var(--pri);font-size:14px;margin-top:5px}
  ...
  .niCalories b{display:block;font-size:20px;color:var(--pri);margin-top:3px;white-space:nowrap}
  ...
  .niSearch input,.niForm input,.niForm select,.niSettings input,.niTextArea,.niSelect,#builderSearch{width:100%;border-radius:16px;border:1px solid var(--line);background:#fafbfc;color:var(--txt);font-weight:800;padding:0 12px;font-size:13px;outline:none;height:46px}
  .niTextArea{height:110px;padding:12px;resize:none;line-height:1.7}
  body.dark .niSearch input,body.dark .niForm input,body.dark .niForm select,body.dark .niSettings input,body.dark .niTextArea,body.dark .niSelect,body.dark #builderSearch{background:#0b1b18}
  .niTabs{display:flex;gap:5px;overflow-x:auto;overflow-y:hidden;background:var(--card);border:1px solid var(--line);border-radius:18px;padding:6px;position:sticky;top:0;z-index:20;scrollbar-width:none;box-shadow:0 8px 18px #0000000a;white-space:nowrap}
  .niTabs::-webkit-scrollbar{display:none}
  .niTabs button{border:0;background:transparent;color:var(--muted);border-radius:13px;padding:8px 13px;font-weight:950;font-size:12px;white-space:nowrap;flex:0 0 auto}
  .niTabs button.on{background:var(--pri);color:#fff}
  .niSubTabs{display:flex;gap:7px;overflow:auto;margin-bottom:10px}
  .niSubTabs button,.niCats button{border:1px solid var(--line);border-radius:999px;background:var(--card);color:var(--txt);padding:8px 12px;font-weight:900;white-space:nowrap}
  .niSubTabs button.on{background:var(--pri);color:#fff}
  ...
  .niMealItem span,.niMealItem em,.niFoodResults span,.niTemplate span{display:block;color:var(--muted);font-size:11px;margin-top:4px;font-style:normal}
  ...
  .niMealItem button,.niFoodRow button{border:0;border-radius:10px;padding:7px 8px;font-weight:900;margin:2px;font-size:11px;background:#f1f5f9;color:#111827}
  .niSettings,.niForm{display:grid;grid-template-columns:repeat(2,1fr);gap:10px}
  ...
  .niFoodList{display:grid;gap:8px;max-height:430px;overflow:auto}
  .niFoodList.compact{max-height:360px}
  ...
  .niCats{display:flex;gap:7px;overflow-x:auto;overflow-y:hidden;margin:10px 0;white-space:nowrap}
  .niCats::-webkit-scrollbar{display:none}
  ...
  .niModalBg,.niPanelBg{position:fixed;inset:0;background:#0007;z-index:10000;display:flex;align-items:flex-end}
  .niModal,.niPanel{background:var(--card);border-radius:26px 26px 0 0;padding:16px;width:100%;max-height:88vh;overflow:auto}
  .niPanel{height:88vh}
  .niModalHead,.niPanelHead{display:flex;justify-content:space-between;align-items:center;margin-bottom:12px}
  .niModalHead h3,.niPanelHead h3{font-size:20px;margin:0;color:var(--txt)}
  ...
  @media(max-width:600px){
    ...
    .niHero h2{font-size:20px}
    .niHero p{font-size:11.5px}
    .niScoreBox{min-width:78px;padding:10px}
    .niScoreBox b{font-size:22px}
    .niExecDash,.niMini,.niKpis,.niQuick,.niFoodResults{grid-template-columns:repeat(2,1fr)}
    ...
    .niPanel{height:92vh;max-height:92vh}
  }`;
