/* Liyaqti Nutrition Intelligence Pro - Premium V20 */
console.log("Liyaqti Nutrition Intelligence Pro V20 loaded");
...
let NS=readStore(NSET,{
  ...
  goalType:"loss",height:162,weight:92,activity:"moderate",age:29,streak:0
});
...
["قهوة سوداء","مشروبات","snack","كوب",250,5,0,1,0,0,0,5,"clean","تقديري","high"],
["لاتيه قليل الدسم","مشروبات","snack","كوب",300,150,10,18,4,0,14,140,"medium","ملصق/تقديري","medium"],
["كابتشينو","مشروبات","snack","كوب",250,120,7,12,5,0,9,110,"medium","تقديري","medium"],
["شاي كرك","مشروبات","snack","كوب",250,180,5,28,6,0,22,120,"high_sugar","تقديري","medium"],
["ماء","مشروبات","snack","كوب",250,0,0,0,0,0,0,0,"clean","يدوي","high"],
...
["بطاط مشوية","كارب","lunch","100g",100,95,2,21,0,2,1,8,"clean","USDA/تقديري","high"],
...
["خبز أبيض","فطور","breakfast","شريحة",35,90,3,17,1,1,2,150,"medium","ملصق/تقديري","medium"],
...
["حليب قليل الدسم","ألبان","snack","كوب",250,105,8,12,3,0,12,120,"clean","ملصق/تقديري","medium"],
...
["دجاج مسلوق","بروتين","lunch","100g",100,155,30,0,3,0,0,80,"clean","تقديري","high"],
["تونة ماء","بروتين","dinner","علبة",120,130,28,0,1,0,0,350,"clean","ملصق/تقديري","high"],
["سلمون مشوي","بروتين","dinner","100g",100,208,25,0,12,0,0,60,"clean","USDA/تقديري","high"],
...
["خيار","خضار","snack","100g",100,15,1,4,0,1,2,2,"clean","USDA/تقديري","high"],
["طماطم","خضار","snack","100g",100,18,1,4,0,1,3,5,"clean","USDA/تقديري","high"],
["بروكلي","خضار","lunch","100g",100,35,3,7,0,3,2,33,"clean","USDA/تقديري","high"],
...
["برتقال","فواكه","snack","حبة",130,62,1,15,0,3,12,0,"clean","USDA/تقديري","high"],
...
let foodLibrary=readStore(NFLIB,null)||defaultFoodLibrary;

/* =========================
   Core Helpers
========================= */
function readStore(k,fallback){try{return JSON.parse(localStorage.getItem(k)||"null")??fallback}catch(e){return fallback}}
...
function timingScore(){
  let t=nToday(); if(!t.length)return 40;
...
function nScore(s=nSum()){
  ...
  return Math.round(cal*.18+protein*.18+water*.13+fiber*.09+sugar*.10+sodium*.14+foodQualityScore()*.12+timingScore()*.06);
}
...
function smartTargets(){
  let w=+NS.weight||92,h=+NS.height||162,age=+NS.age||29;
  ...
  <div class="ni">
    <section class="niHero">
      <div class="niHeroText">
        <div class="niEyebrow">LIYAQTI NUTRITION INTELLIGENCE PRO V20</div>
...
function renderNutritionTab(){
  ...
  const views={
    command:commandTab,overview:overviewTab,meals:mealsTab,builder:builderTab,restaurants:restaurantsTab,
    radar:radarTab,weekly:weeklyTab,predict:predictTab,simulator:simulatorTab,coach:coachTab,
    health:healthTab,patterns:patternsTab,grocery:groceryTab,timing:timingTab,protein:proteinTab,
    calculator:calculatorTab,library:libraryTab,settings:settingsTab
  };
...
function commandTab(){
  ...
  <section class="niStrategic">
    <div class="niStrategicHead">
      <div><span>V20 COMMAND CENTER</span><h3>🧭 مركز القرار الغذائي</h3></div>
      <b>${nScore()}%</b>
    </div>
    ${strategicNutritionAnalysis()}
  </section>
  <section class="niGrid2">
    <div class="niCard"><div class="niCardHead"><div><small>Nutrition Twin</small><h3>🧬 توأمك الغذائي</h3></div></div><div class="niRecommend">${nutritionTwinText()}</div></div>
    <div class="niCard"><div class="niCardHead"><div><small>7-Day Direction</small><h3>🔮 اتجاه 7 أيام</h3></div></div><div class="niRecommend">${sevenDayScenarioText()}</div></div>
  </section>
  <section class="niCard"><div class="niCardHead"><div><small>Executive Summary</small><h3>📌 ملخص تنفيذي</h3></div></div><div class="niRecommend">${executiveSummary()}</div></section>`;
}
...
function mealsTab(){
  let meals=nToday();
  return `
  <section class="niCard niAction"><div><small>Today Meals</small><h3>🍽️ وجبات اليوم</h3><p>إدارة، تقييم، وبدائل ذكية لكل وجبة.</p></div><button onclick="openMealModal()">+ إضافة</button></section>
...
function builderTab(){
  ...
  <section class="niCard">
    ...
    <div class="niCats">${cats.slice(0,14).map(c=>`<button onclick="builderCat('${c}')">${c}</button>`).join("")}</div>
  ...
  <section class="niCard"><div class="niCardHead"><div><small>Saved Meals</small><h3>وجبات مركبة محفوظة</h3></div></div>
...
function radarTab(){
  return `<section class="niCard"><div class="niCardHead"><div><small>Nutrition Risk Radar</small><h3>📡 رادار المخاطر</h3></div></div>
  ${riskScores().map(x=>`<div class="niGoal"><div><b>${x.name}</b><span>${x.v}%</span></div><p><i style="width:${Math.min(100,x.v)}%"></i></p></div>`).join("")}</section>
...
function weeklyTab(){
  ...
  return `<section class="niCard"><div class="niCardHead"><div><small>Executive Weekly Report</small><h3>📈 التقرير الأسبوعي التنفيذي</h3></div></div>
    <div class="niKpis">
...
function predictTab(){
  return `
  <section class="niCard"><div class="niCardHead"><div><small>Predictive Nutrition</small><h3>🔮 توقع تأثير اليوم</h3></div></div><div class="niRecommend">${weightImpactForecast()}</div></section>
  <section class="niCard"><div class="niCardHead"><div><small>Plateau Detector</small><h3>🧱 كشف ثبات الوزن</h3></div></div><div class="niRecommend">${plateauDetector()}</div></section>
  <section class="niCard"><div class="niCardHead"><div><small>Goal ETA</small><h3>🎯 توقع الوصول للهدف</h3></div></div><div class="niRecommend">${goalEtaText()}</div></section>`;
}

function simulatorTab(){
  return `
  <section class="niCard"><div class="niCardHead"><div><small>What If Simulator</small><h3>🧪 محاكي القرارات</h3></div></div>
    <div class="niQuick">
  ...
  <section class="niCard"><div class="niCardHead"><div><small>7-Day Scenario</small><h3>📅 محاكاة 7 أيام</h3></div></div><div class="niRecommend">${sevenDayScenarioText()}</div></section>`;
}

function coachTab(){
  return `
  <section class="niCard"><div class="niCardHead"><div><small>AI Coach</small><h3>🤖 مدرب التغذية الذكي</h3></div></div>
    <div class="niForm"><div style="grid-column:1/-1"><label>اسأل المدرب</label><input id="coachAsk" placeholder="مثال: شو آكل الحين؟ أو بروتيني ناقص؟"></div></div>
    <button class="niMainBtn" onclick="coachAnswer()">اسأل</button>
    <div id="coachBox" class="niRecommend">${coachDefault()}</div>
  </section>
  <section class="niCard"><div class="niCardHead"><div><small>Personal Rules</small><h3>⚙️ قواعدك الشخصية</h3></div></div>
    <div class="niQuick"><button onclick="addRule()">+ إضافة قاعدة</button><button onclick="clearRules()">مسح القواعد</button></div>
...
function healthTab(){
  let s=nSum();
  return `<section class="niCard"><div class="niCardHead"><div><small>Health Intelligence</small><h3>🩺 طبقة الصحة الذكية</h3></div></div>
    <div class="niKpis">
  ...
  <section class="niCard"><div class="niCardHead"><div><small>Micronutrient Lite</small><h3>🧬 معادن وفيتامينات تقديرية</h3></div></div><div class="niRecommend">${microLite()}</div></section>`;
}

function patternsTab(){
  return `<section class="niCard"><div class="niCardHead"><div><small>Diet Pattern Detection</small><h3>🧬 تحليل النمط الغذائي</h3></div></div><div class="niRecommend">${patternText()}</div></section>
  <section class="niCard"><div class="niCardHead"><div><small>Usual Meals</small><h3>🔁 وجباتك المعتادة</h3></div></div>${usualMeals()}</section>`;
}

function groceryTab(){
  return `<section class="niCard"><div class="niCardHead"><div><small>Smart Grocery List</small><h3>🛒 قائمة مشتريات ذكية</h3></div></div>
  <div class="niFoodResults">${groceryList().map(x=>`<button><b>${x}</b><span>مقترح حسب نواقصك</span></button>`).join("")}</div></section>`;
}

function timingTab(){
  let by={breakfast:nSum(nToday().filter(x=>x.meal==="breakfast")),lunch:nSum(nToday().filter(x=>x.meal==="lunch")),dinner:nSum(nToday().filter(x=>x.meal==="dinner")),snack:nSum(nToday().filter(x=>x.meal==="snack"))};
  return `<section class="niCard"><div class="niCardHead"><div><small>Meal Timing Analysis</small><h3>⏱️ تحليل توقيت الوجبات</h3></div></div>
  <div class="niKpis"><div><span>فطور</span><b>${by.breakfast.cal}</b></div><div><span>غداء</span><b>${by.lunch.cal}</b></div><div><span>عشاء</span><b>${by.dinner.cal}</b></div><div><span>سناك</span><b>${by.snack.cal}</b></div></div>
  <div class="niRecommend">${timingAdvice(by)}</div></section>
  <section class="niCard"><div class="niCardHead"><div><small>Portion Guide</small><h3>🥣 دليل الكميات</h3></div></div>
  <div class="niFoodResults"><button><b>نص صحن رز</b><span>تقريباً 120-150g</span></button><button><b>صحن رز كامل</b><span>250-300g</span></button><button><b>ملعقة روب</b><span>15-20g</span></button><button><b>كوب مشروب</b><span>250-300ml</span></button></div></section>`;
}

function proteinTab(){
  let s=nSum(),need=Math.max(0,NS.protein-s.p);
  let sources=nToday().filter(x=>x.p>0).sort((a,b)=>b.p-a.p).slice(0,5);
  return `<section class="niCard"><div class="niCardHead"><div><small>Protein Center</small><h3>🥩 مركز البروتين</h3></div><b>${pct(s.p,NS.protein)}%</b></div>
  <div class="niProgress"><i style="width:${pct(s.p,NS.protein)}%"></i></div>
  <div class="niRecommend">وصلت ${s.p}g من ${NS.protein}g. المتبقي ${need}g.<br><br>${need>40?"البروتين ناقص بوضوح. أضف وجبة بروتين رئيسية.":need>15?"أضف سناك بروتين خفيف.":"البروتين ممتاز تقريباً."}</div></section>
  <section class="niCard"><div class="niCardHead"><div><small>Top Sources</small><h3>أفضل مصادر البروتين اليوم</h3></div></div>${sources.length?sources.map(x=>`<div class="niTemplate"><b>${x.name}</b><span>${x.p}g بروتين • ${x.cal} سعرة</span></div>`).join(""):`<div class="niEmpty small">لا توجد مصادر بروتين.</div>`}</section>`;
}

function calculatorTab(){
  return `<section class="niCard"><div class="niCardHead"><div><small>Natural Language Logging</small><h3>🧮 الحاسبة النصية الذكية</h3></div></div>
  ...
  <div id="mealCalcResult"></div></section>`;
}

function libraryTab(){
  let cats=[...new Set(foodLibrary.map(x=>x.cat||"عام"))].sort();
  return `<section class="niCard niAction"><div><small>Food Database Pro</small><h3>📚 مكتبة الأطعمة الاحترافية</h3><p>تصنيف، مصدر، ثقة، وتعديل مباشر.</p></div><button onclick="openFoodModal()">+ طعام</button></section>
...
function settingsTab(){
  return `<section class="niCard"><div class="niCardHead"><div><small>Adaptive Targets</small><h3>🎯 أهداف ذكية تتكيف مع تقدمك</h3></div></div>
  ...
  <button class="niMainBtn" onclick="applyGoalPreset()">حساب أهداف تلقائية</button></section>
  <section class="niCard"><div class="niCardHead"><div><small>Manual Targets</small><h3>تعديل يدوي</h3></div></div>
  ...
  <button class="niMainBtn" onclick="saveNutritionSettings()">حفظ الأهداف</button></section>`;
}
...
function mealScoreLabel(x){let s=mealScore(x);return s>=85?"ممتازة":s>=70?"جيدة":s>=50?"متوسطة":"تحتاج تحسين"}
...
function mealsList(meals){
  return ["breakfast","lunch","dinner","snack"].map(g=>{
    ...
    let total=nSum(list);
    return `<section class="niMeal"><div class="niMealHead"><b>${mealName(g)}</b><span>${total.cal} سعرة • P ${total.p}g</span></div>
    ${list.map(x=>`<div class="niMealItem"><div><b>${x.name}</b><span>${x.cal} kcal • P ${x.p} • C ${x.c} • F ${x.f}</span><em>⭐ ${mealScore(x)}% • ${mealScoreLabel(x)} • ${qualityName(x.quality)} • ${confName(x.confidence)}</em>${smartSwapForMeal(x)}</div><div class="niMealActions"><button onclick="editNutritionMeal(${x.id})">تعديل</button><button onclick="copyMealToToday(${x.id})">نسخ</button><button onclick="deleteNutritionMeal(${x.id})">حذف</button></div></div>`).join("")}</section>`;
  }).join("");
}
...
function strategicNutritionAnalysis(){
  ...
  return `<div class="niStrategyGrid">
  ...
  <div class="niStrategyList"><div>✅ بروتين بكل وجبة.</div><div>💧 الماء جزء من الخطة.</div><div>🌾 راقب الصوديوم قبل الحكم على الوزن.</div><div>📊 القرار الأسبوعي أهم من يوم واحد.</div></div>`;
}
...
function executiveSummary(){
  ...
  return `الدرجة: ${nScore(s)}%.<br>المسار: ${goalTrack()}.<br>الخطر الأكبر: ${topRisk()}.<br>أفضل فرصة: ${bestOpportunity()}.<br>أفضل وجبة: ${best?best.name:"لا توجد"}.<br>أضعف وجبة: ${worst?worst.name:"لا توجد"}.<br>الوزن: ${getWeightTrendText()}`;
}
...
function plateauDetector(){
  ...
  let last=w.slice(-4).map(x=>x.weight),max=Math.max(...last),min=Math.min(...last),s=nSum();
...
function runScenario(type){
  let s=nSum(),text="";
  if(type==="remove_fried")text="تقليل المقليات قد يخفض تقريباً 250–500 سعرة ودهون عالية في اليوم.";
  if(type==="add_steps")text="زيادة 3000 خطوة قد ترفع الحرق التقريبي 100–180 سعرة.";
  if(type==="swap_shawarma")text="استبدال الشاورما بتونة/دجاج + سلطة قد يخفض الصوديوم والدهون.";
  if(type==="add_protein")text=`إضافة 25–30g بروتين ترفع الشبع وتحسن مسار هدفك، خصوصاً لو البروتين الحالي ${s.p}g.`;
...
function coachAnswer(){
  let q=normalizeArabicText(document.getElementById("coachAsk").value||""),s=nSum(),ans=coachDefault();
...
function addRule(){let r=prompt("اكتب قاعدة شخصية للمدرب، مثال: لا تقترح سمك");if(!r)return;NR.push(r);nSave();renderNutrition()}
function clearRules(){if(!confirm("مسح كل القواعد؟"))return;NR=[];nSave();renderNutrition()}
...
function microLite(){
  ...
  let dairy=t.some(x=>x.cat==="ألبان"),fruits=t.some(x=>x.cat==="فواكه"),veg=t.some(x=>x.cat==="خضار");
...
/* =========================
   Builder / Restaurants / Library
========================= */
function builderCat(c){
  let box=document.getElementById("builderResults"); if(!box)return;
...
function addBuilderItem(i){
  let x=foodLibrary[i],amount=prompt(`كمية ${x.name}؟`,x.grams);
...
function renderBuilderBox(){
  ...
  return `<div class="niCalcResult"><h4>الوجبة الحالية</h4>
  ...
  <button class="niMainBtn" onclick="saveBuilderMeal()">حفظ/إضافة الوجبة</button></div>`;
}

function removeBuilderItem(i){builderItems.splice(i,1);let box=document.getElementById("builderBox");if(box)box.innerHTML=renderBuilderBox()}

function saveBuilderMeal(){
  if(!builderItems.length)return alert("أضف مكونات أولاً.");
  let name=prompt("اسم الوجبة المركبة","وجبتي المركبة"); if(!name)return;
  let total=nSum(builderItems);
...
function useBuiltMeal(id){
  let b=NB.find(x=>x.id===id); if(!b)return;
  b.items.forEach((x,i)=>addMealObject(x.food,x.amount,Date.now()+i,x));
  nSave();nTab="meals";renderNutrition();
}

function restaurantMealScore(x){return (x.p*2)-x.f-(x.sodium/100)+(x.quality==="clean"?20:0)-(x.quality==="high_sodium"?20:0)}

function restaurantList(c){
  let box=document.getElementById("restaurantBox"); if(!box)return;
  let list=foodLibrary.filter(x=>x.cat===c).sort((a,b)=>restaurantMealScore(b)-restaurantMealScore(a));
  box.innerHTML=`<div class="niFoodList">${foodRows(list)}</div><div class="niRecommend">أفضل اختيار لهدفك: ${list[0]?list[0].name:"غير متوفر"}. ركز على الأقل صوديوم والأعلى بروتين.</div>`;
}

function foodRows(list){
  return list.map(x=>`<div class="niFoodRow"><div><b>${x.name}</b><span>${x.cal} سعرة • P ${x.p} • ${x.unit} • ${x.cat} • ${qualityName(x.quality)} • ${x.source} • ${confName(x.confidence)}</span></div><div><button onclick="quickAddWithAmount(${foodLibrary.indexOf(x)})">إضافة</button><button onclick="openFoodModal(${foodLibrary.indexOf(x)})">تعديل</button><button onclick="deleteFoodItem(${foodLibrary.indexOf(x)})">حذف</button></div></div>`).join("");
}
...
function calculateMealText(){
  let raw=document.getElementById("mealCalcText").value||"",text=normalizeArabicText(raw),result=document.getElementById("mealCalcResult");
  calcItems=[];
  ...
  chunks.forEach(chunk=>{
    foodLibrary.forEach(food=>{
      let hit=foodAliases(food).find(a=>chunk.includes(a)); if(!hit)return;
      let amount=food.grams,nums=chunk.match(/\d+(\.\d+)?/g);
      if(nums&&nums.length){
        let n=+nums[0];
      ...
      if(!calcItems.some(z=>z.food.name===food.name&&z.amount===amount))calcItems.push({food,amount,...scaled});
    });
  });
  if(!calcItems.length){result.innerHTML=`<div class="niEmpty small">ما قدرت أتعرف على الأكلات. أضفها من المكتبة أو اكتب اسم أوضح.</div>`;return}
  let total=nSum(calcItems);
  result.innerHTML=`<div class="niCalcResult"><h4>النتيجة التقريبية</h4><div class="niKpis"><div><span>السعرات</span><b>${total.cal}</b></div><div><span>بروتين</span><b>${total.p}g</b></div><div><span>كارب</span><b>${total.c}g</b></div><div><span>دهون</span><b>${total.f}g</b></div></div>${calcItems.map(x=>`<p>${x.food.name} • ${x.amount}g تقريباً • ${x.cal} سعرة • ${x.food.source}</p>`).join("")}<button class="niMainBtn" onclick="addCalculatedMeal()">إضافة الوجبة لليوم</button></div>`;
}

function addCalculatedMeal(){
  calcItems.forEach((x,i)=>addMealObject(x.food,x.amount,Date.now()+i,x));
  calcItems=[];nSave();nTab="meals";renderNutrition();
}
...
function openMealModal(id=null){
  ...
  if(!modal)return;
  modal.innerHTML=`<div class="niModalBg"><div class="niModal"><div class="niModalHead"><h3>${x?"تعديل وجبة":"إضافة وجبة"}</h3><button onclick="closeMealModal()">×</button></div><div class="niForm">
  ...
  </div><button class="niMainBtn" onclick="saveMealFromModal()">حفظ الوجبة</button></div></div>`;
...
function openFoodModal(i=null){
  ...
  if(!modal)return;
  modal.innerHTML=`<div class="niModalBg"><div class="niModal"><div class="niModalHead"><h3>${x?"تعديل طعام":"إضافة طعام"}</h3><button onclick="closeFoodModal()">×</button></div><div class="niForm">
  ...
  </div><button class="niMainBtn" onclick="saveFoodFromModal()">حفظ الطعام</button></div></div>`;
  if(x){
...
function saveFoodFromModal(){
  let item={
    id:"f"+Date.now(),name:document.getElementById("fName").value||"طعام",
    cat:document.getElementById("fCat").value||"عام",meal:document.getElementById("fMeal").value,
    unit:document.getElementById("fUnit").value||"100g",grams:+document.getElementById("fGrams").value||100,
    cal:+document.getElementById("fCal").value||0,p:+document.getElementById("fP").value||0,
    c:+document.getElementById("fC").value||0,f:+document.getElementById("fF").value||0,
    fiber:+document.getElementById("fFiber").value||0,sugar:+document.getElementById("fSugar").value||0,
    sodium:+document.getElementById("fSodium").value||0,source:document.getElementById("fSource").value||"تقديري",
    confidence:document.getElementById("fConfidence").value||"medium",quality:document.getElementById("fQuality").value||"medium"
  };
...
function injectNutritionStyle(){
  ...
  .niHero:before{content:"";position:absolute;inset:-40px auto auto -40px;width:140px;height:140px;border-radius:50%;background:#ffffff18}.niHeroText{position:relative;z-index:1}.niEyebrow{font-size:10px;letter-spacing:.8px;color:#ccfbf1;font-weight:900}
  .niHero h2{font-size:21px;margin:5px 0 4px;white-space:nowrap;font-weight:950}.niHero p{font-size:12px;line-height:1.6;margin:0;color:#ecfeff;font-weight:650;max-width:520px}
  ...
  .niAction{display:flex;justify-content:space-between;align-items:center}.niAction p{color:var(--muted);font-size:12px;margin:5px 0 0}.niQuick button,.niFoodResults button,.niSearchHead button,.niAction button{border:1px solid var(--line);background:var(--card);border-radius:15px;padding:11px;font-weight:950;color:var(--txt);font-size:12px;text-align:start}.niMainBtn{border:0;border-radius:16px;background:linear-gradient(135deg,#0f766e,#14b8a6);color:#fff;padding:11px 15px;font-weight:950;font-size:13px;width:100%;margin-top:12px;height:48px}
  .niMeal{background:var(--card);border:1px solid var(--line);border-radius:19px;overflow:hidden;margin-top:10px;box-shadow:0 8px 18px #00000008}.niMealHead{display:flex;justify-content:space-between;background:#eefaf7;color:#0f766e;padding:12px;font-weight:950;font-size:13px}.niMealItem{display:flex;justify-content:space-between;gap:8px;padding:11px;border-top:1px solid var(--line)}.niMealItem b{font-size:13px;color:var(--txt)}.niMealItem span,.niMealItem em,.niFoodResults span,.niTemplate span{display:block;color:var(--muted);font-size:11px;margin-top:4px;font-style:normal}.niSwap{display:inline-block;margin-top:7px;background:#ecfeff;color:#0f766e;border:1px solid #99f6e4;border-radius:999px;padding:5px 8px;font-weight:900}.niMealActions{text-align:left;white-space:nowrap}.niMealItem button,.niFoodRow button{border:0;border-radius:10px;padding:7px 8px;font-weight:900;margin:2px;font-size:11px;background:#f1f5f9;color:#111827}
  ...
  .niFoodList{display:grid;gap:8px}.niFoodRow{display:flex;justify-content:space-between;gap:8px;border:1px solid var(--line);border-radius:16px;padding:10px;background:#f8faf9}.niFoodRow span{display:block;color:var(--muted);font-size:11px;margin-top:4px}.niCats{display:flex;gap:7px;overflow:auto;margin-bottom:10px}.niCats button{border:1px solid var(--line);border-radius:999px;background:var(--card);color:var(--txt);padding:8px 12px;font-weight:900;white-space:nowrap}
