/* =========================================================
   Liyaqti Goal Command Center V70.1
   Clean User Names
   File: js/goal.js
========================================================= */
...
const META={
 loss:{icon:"⚖️",name:"خسارة الوزن",title:"مركز خسارة الوزن",desc:"مركز خسارة الوزن: نزول، سعرات، خطوات، ثبات، وتوقع الوصول.",focus:"نزول صحي وثابت",color:"#0f766e"},
 gain:{icon:"📈",name:"زيادة وزن",title:"مركز زيادة الوزن",desc:"مركز زيادة الوزن: فائض سعرات، بروتين، وزن مكتسب، واستمرارية.",focus:"زيادة وزن نظيفة",color:"#b45309"},
 run:{icon:"🏃",name:"اختبار رياضي",title:"مركز الاختبار الرياضي",desc:"مركز الاختبار الرياضي: مسافة، زمن، Pace، جاهزية، وخطة جري.",focus:"تحسين الزمن والجاهزية",color:"#2563eb"},
 steps:{icon:"👣",name:"تحدي خطوات",title:"مركز تحدي الخطوات",desc:"مركز تحدي الخطوات: خطوات اليوم، المتوسط، أيام النجاح، والالتزام.",focus:"رفع النشاط اليومي",color:"#0891b2"},
 gym:{icon:"💪",name:"بناء عضلات",title:"مركز بناء العضلات",desc:"مركز بناء العضلات: مقاومة، بروتين، جسم، التزام، وكتلة عضلية.",focus:"بناء عضل وتحسين الجسم",color:"#7c3aed"},
 custom:{icon:"🎯",name:"هدف مخصص",title:"مركز الهدف المخصص",desc:"مركز الهدف المخصص: تقدم، مهام، موعد، وملاحظات.",focus:"هدفك الخاص",color:"#be123c"}
};
...
if(m==="steps"||m==="run"||m==="custom"){start=n(S.start,93);target=n(S.goal,75)}
...
function css(){
if(document.getElementById("goalV701Style"))return;
...
addH("تم التبديل إلى "+META[t].name);
renderGoalV70();
};
...
addH("تم تسجيل اليوم في "+meta().name);
renderGoalV70();
if(typeof render==="function")try{render()}catch(e){}
};
...
S.sportTest=t;saveS();setProfile("run",t);addH("تم حفظ إعدادات الاختبار الرياضي");renderGoalV70();
};
...
<button class="g70Btn" onclick="goalV70SaveSport()">حفظ إعدادات الاختبار</button>`;
if(t==="steps")return `
...
<button class="g70Btn" onclick="goalV70SaveGoal()">حفظ إعدادات الخطوات</button>`;
if(t==="custom")return `
...
<button class="g70Btn" onclick="goalV70SaveGoal()">حفظ إعدادات ${meta().name}</button>`;
}

function renderGoalV70(){
css();
let c=calc(),mt=meta(c.m),b=body(),hm=heroMetrics(c),ai=aiList(c);
...
<details><summary>🎯 اختيار الهدف</summary>
...
<details><summary>🧩 إعدادات ${mt.name}</summary><div class="g70List">${profileSettingsHTML(c)}</div></details>
...
${c.m==="gym"?`
<details open><summary>📏 قياسات الجسم</summary>
<div class="g70List">
<div class="g70Grid">
...
<button class="g70Btn" onclick="goalV70SaveBody()">حفظ قياسات الجسم</button>
...
<div class="g70List">
<input id="g70Note" class="g70Input" placeholder="اكتب ملاحظة لهذا الهدف">
<button class="g70Btn" onclick="goalV70AddNote()">حفظ الملاحظة</button>
${noteList.length?noteList.map(x=>`<div class="g70Item">📅 ${x.d}<br>${x.text}</div>`).join(""):`<div class="g70Item">لا توجد ملاحظات لهذا الهدف.</div>`}
</div>
</details>
