// ========================================
// Liyaqti Advanced Reports Center
// Phase 1
// ========================================
...
function reportFilteredSteps(){
  let arr = reportAllSteps();
  if(liyaqtiReportRange === "all") return arr;
...
function exportAdvancedCSV(){
  ...
  let stepsMap = {};
  reportAllSteps().forEach(x=>stepsMap[x.d]=x.steps);
...
function renderAdvancedReports(){
  ...
  let weights = reportFilteredWeights();
  let steps = reportFilteredSteps();
  ...
  page.innerHTML = `
    ...
    <div class="card">
      ...
      <div style="display:flex;gap:10px;flex-wrap:wrap;margin-top:14px">
        ...
        <button class="miniBtn" onclick="exportAdvancedCSV()">تصدير CSV</button>
      </div>
    </div>

    <div class="grid kpis">
      <div class="card"><div class="muted">الوزن الحالي</div><div class="num">${current || "--"} kg</div></div>
      <div class="card"><div class="muted">نسبة الإنجاز</div><div class="num">${progress}%</div></div>
      <div class="card"><div class="muted">المفقود</div><div class="num">${lost.toFixed(1)} كجم</div></div>
      <div class="card"><div class="muted">المتبقي</div><div class="num">${remaining.toFixed(1)} كجم</div></div>
      <div class="card"><div class="muted">مجموع الخطوات</div><div class="num">${totalSteps}</div></div>
      ...
      <div class="card"><div class="muted">السعرات المحروقة</div><div class="num">${Math.round(totalBurn)}</div></div>
      <div class="card"><div class="muted">مسافة النشاط</div><div class="num">${totalKm.toFixed(1)} كم</div></div>
      <div class="card"><div class="muted">مدة النشاط</div><div class="num">${Math.round(totalMin)} د</div></div>
    </div>
    ...
    <div class="card">
      ...
      <div class="coachBox">
        ...
        <div class="coachItem">⚖️ وزنك الحالي ${current || "--"} كجم.</div>
        <div class="coachItem">👣 متوسط خطواتك ${avgSteps} خطوة.</div>
        <div class="coachItem">🔥 حرقت من الأنشطة ${Math.round(totalBurn)} سعرة.</div>
    ...
    <div class="card">
      ...
      <p class="muted">
        هذه الصفحة صارت مركز تقارير داخلي. لاحقاً بنضيف Export Excel و PDF وتقرير شامل جاهز للعرض والمشاركة.
      </p>
    </div>
  `;
